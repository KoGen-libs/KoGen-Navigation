...
import kz.evko.navigation.demo.featuresettings.navigation.ActionToSettingsDetails
...
//
// Wired with a real navigateSafety(NavigationAction) call (not just static text) specifically so
// this tab's own internal navigation can be clicked through on-device, not just compiled.
@KoGenTab(graph = "settingsTab", startDestination = true)
@Composable
fun SettingsScreen(navController: NavHostController) {
    Column {
        Text("Settings (demoFeatureSettings)")
        Button(onClick = { navController.navigateSafety(ActionToSettingsDetails) }) {
            Text("Go to details")
        }
    }
}
