 ...
 * graph, with [route] already filled in from its own `graph` name. Pass it to
 * `NavHostController.navigateSafety(TabNavigationAction)` in `NavigationExtensions.kt`.
 *
 * Deliberately **not** related to [NavigationAction] at all (no shared supertype either way) -
 * a tab's route only makes sense through that overload's `popUpTo`/`saveState`/`launchSingleTop`/
